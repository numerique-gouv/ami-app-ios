...
extension HomeUserScripts: WebViewUserScriptsProtocol {
    ...
    func printLog(_ message: WKScriptMessage) {
        ...
        switch level {
        case "error":
            AppLog.view.error("\(AppLog.logHeader(caller: self, function: #function)) ❌ ERROR: \(logMessage)")
        case "warn":
            AppLog.view.warning("\(AppLog.logHeader(caller: self, function: #function)) ⚠️ WARN: \(logMessage)")
        case "info":
            AppLog.view.info("\(AppLog.logHeader(caller: self, function: #function)) ℹ️ INFO: \(logMessage)")
        case "debug":
            AppLog.view.debug("\(AppLog.logHeader(caller: self, function: #function)) 🔍 DEBUG: \(logMessage)")
        default:
            AppLog.view.notice("\(AppLog.logHeader(caller: self, function: #function)) 📝 LOG: \(logMessage)")
        }
    }
    ...
    func processMessage(_ message: WKScriptMessage, for viewModel: SwiftUIWebView.ViewModel) {
        ...
        if let messageBody = message.body as? [String: Any],
           let eventName = messageBody["event"] as? String {
            ...
            AppLog.view.notice("\(AppLog.logHeader(caller: self, function: #function)) Event received: \(eventName) - \(String(describing: data))")
